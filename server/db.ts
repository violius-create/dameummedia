...
export async function getServiceItems(limit = 100, offset = 0) {
  ...
  return db.select().from(serviceItems).where(eq(serviceItems.status, 'published')).orderBy(serviceItems.order, desc(serviceItems.createdAt)).limit(limit).offset(offset).execute();
}
