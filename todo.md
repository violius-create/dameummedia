...
## 모바일 섹션 2/3 여백, 높이, 타이틀 수정 (2026-02-13 추가)
...
## 모바일 레이아웃 이슈 수정 (2026-02-14 추가)
- [x] 모바일 섹션 3 (Making Film) 높이가 낮은 문제 해결 (h-[350px]→h-[280px]로 섹션 2와 동일하게 조정)
- [x] 인스타그램 섹션 아래 큰 여백 제거 (모바일 h-[1400px]→h-[580px]로 축소)
- [x] 게시물 제목이 길면 좌우 스크롤 발생하는 문제 해결 (truncate 적용, 모바일 전용 레이아웃 추가, body overflow-x:hidden)

## 모바일 레이아웃 이슈 수정 2차 (2026-02-14 추가)
...
## UI 개선 요청 2차 (2026-02-14)
- [x] Featured 카드 제목 사이즈 1.5배 확대
- [x] Featured 카드 제목 호버 시 오렌지색으로 변경
- [x] 모든 게시판 view 페이지 가로 넓이를 리스트 페이지와 동일하게 확대
- [x] 모든 게시판 수정 페이지 가로 넓이를 리스트 페이지와 동일하게 확대
- [x] PostDetail 하단 '다른 게시물 보기'를 리스트 화면과 같은 개수로 조정 + 페이지네이션 표시
- [x] 섹션 2,3 게시글 수정 시 관리자 페이지로 넘어가는 문제 해결 (직접 수정 페이지로 이동)
- [x] 메인화면 화살표 눌러도 더 이동하지 않는 문제 해결 (limit 6 → 50으로 증가)


## 섹션1 풀스크린 및 히어로 텍스트 로테이션 (2026-02-14)
- [x] 섹션1 세로 높이를 화면 꽉 차게 (100vh)
- [x] 스크롤 시 섹션1이 서서히 페이드아웃되면서 섹션2 이하가 올라오는 효과
- [x] 히어로 텍스트 3개를 관리자 페이지에서 설정 가능하도록 DB/API 추가
- [x] 히어로 텍스트 2초 간격 자동 로테이션 (반복 시간도 관리자 설정 가능)
- [x] 관리자 페이지에 히어로 텍스트 로테이션 설정 UI 추가


## 섹션1 모션 개선 및 관리자 탭 줄바꿈 (2026-02-14)
- [x] 섹션1 텍스트 세로 중앙 정렬
- [x] 텍스트 전환 시 멋진 모션 효과 추가 (블러+스케일+슬라이드+파란 라인 확장 애니메이션)
- [x] 스크롤 시 섹션2가 겹쳐지면서 올라오는 모션 효과 (parallax + 둥근 모서리 + 그림자)
- [x] 관리자 대시보드 탭 제목이 한 줄에 안 되어도 다 보이도록 줄바꿈 처리 (flex-wrap)


## 섹션1 잔상 수정 및 애니메이션 선택 기능 (2026-02-14)
...
- [x] 스크롤 인디케이터 크기 확대(32→48px) 및 위치를 더 위로 조정(bottom-10→bottom-24)


## 대시보드 편집 기능 및 스크롤 수정 (2026-02-14)
- [x] Information 페이지 각 항목 내용을 대시보드에서 수정 가능하게 구현 - DB 테이블 + 라우터 + 관리 페이지 추가
...
## dameum.kr DB 마이그레이션 (2026-02-14)
- [x] 이전 예약 데이터(326건) 필드 구조 분석 및 새 테이블 설계
- [x] 포트폴리오(206건), 공연실황(187건), 공지(6건), 사진(13건) 등 게시글 마이그레이션 - 395건 완료
- [x] 사업실적(14건), 회사소개(4건), 인사말(9건) 등 정보 페이지 마이그레이션 - 27건 완료
- [x] 예약 데이터(326건) 이전 필드 구조 그대로 새 테이블로 이전 - 293건 완료
- [x] 현재 예약 게시판에서 이전 예약 데이터 접근 가능하게 구현 - 기존 reservations 테이블에 매핑
- [x] 마이그레이션 결과 검증 - 예약 293건, 게시글 395건, 정보 27건 확인 완료

## portfoliodata → film 카테고리 변경 (2026-02-14)
- [x] 마이그레이션된 portfolio 카테고리 게시글 196건을 film(Making Film)으로 변경 완료

## 관리자게시판 구현 (2026-02-14)
- [x] 이전 사이트 working 테이블 데이터 분석 - 4건 중 본글 2건 확인
- [x] 관리자 전용 게시판 카테고리(admin_board) 추가 - DB enum 확장
- [x] 관리자게시판 리스트/상세/작성/수정 페이지 구현 - AdminBoard.tsx
- [x] 상단 네비게이션에 관리자만 볼 수 있는 [관리자게시판] 메뉴 추가 - Shield 아이콘
- [x] working 데이터 마이그레이션 - 2건 이전 완료
- [x] 테스트 및 검증 - DB 확인 완료


## 버그 수정 (2026-02-14)
- [x] Information 게시판에서 주요경력/주요실적 항목 수정 불가 문제 해결 - DB 중복 데이터 정리 (sectionKey별 1건만 유지)
- [x] 게시판 제목 클릭 시 하드코딩된 값이 잠깐 보이는 플래시 현상 수정 - fallback 텍스트 빈 문자열로 변경
- [x] 메인화면/기타 배경 이미지도 같은 플래시 현상 수정 - localStorage 로드 제거, 초기값 빈 문자열
- [x] 관리자게시판에 마이그레이션된 working 데이터가 표시되지 않는 문제 해결 - 확인 결과 정상 동작 (관리자 인증 필요)

## 관리자게시판 본문 내용 누락 문제 (2026-02-14)
- [x] 관리자게시판 "정보", "프로그래밍" 게시글의 본문 내용이 비어있는 문제 확인 및 수정
- [x] DB에서 admin_board 게시글의 content 필드 데이터 확인 - ca_name이 제목으로 잘못 매핑됨
- [x] 마이그레이션된 working 데이터의 content 복구 - 844건 전체 재마이그레이션 완료

## 관리자게시판 개선 및 Footer 소셜미디어 (2026-02-14)
- [x] 관리자게시판 원본 작성날짜(wr_datetime) 반영 - 844건 모두 원본 날짜로 업데이트 완료
- [x] 관리자게시판 UI: 날짜를 제목과 같은 줄에 표기, 줄 간격 축소
- [x] Footer 소셜미디어에 유튜브/인스타 항목 추가 및 링크 연결
- [x] Footer 소셜미디어 관리자 수정 기능 구현 (AdminFooterSettings 페이지)

## 관리자게시판 에디터 및 Notice UI 개선 (2026-02-14)
- [x] 관리자게시판 글쓰기 페이지에 리치 텍스트 에디터 적용 (RichTextEditor 컴포넌트 재사용)
...
## 추가 수정 3건 (2026-02-15)
- [x] 예약게시판 글쓰기 페이지에 파일첨부 버튼 추가 (파란색 대시 테두리 강조, S3 업로드 연동)
- [x] 메인화면 reservation/notice latest 섹션 구분선이 보이지 않는 문제 수정 (border-[#d1d5db] 명시적 색상 적용)
- [x] Price 페이지 Professional 옵션의 '추천' 글자 삭제 및 세 옵션 박스 크기 동일하게 통일 (scale-105 제거)

## 인스타 섹션 및 스크롤 개선 (2026-02-15)
- [x] 인스타 섹션 표시 개수 관리자 설정 가능하게
- [x] 인스타 섹션 게시물이 표시 개수보다 많을 경우 좌우 스크롤 버튼 추가
- [x] 섹션2 좌우 스크롤 이동 길이를 한 게시물씩 좌측에 맞도록 조정
- [x] 섹션3 좌우 스크롤 이동 길이를 한 게시물씩 좌측에 맞도록 조정
- [x] 인스타 섹션 좌우 스크롤 이동 길이를 한 게시물씩 좌측에 맞도록 조정

## 버그 수정 4건 (2026-02-15 추가)
- [x] 인스타 표시 개수 10개 설정인데 메인화면에 5개만 보이는 문제 수정
- [x] 인스타 게시물이 표시 개수보다 많은데 좌우 스크롤 화살표가 안 나오는 문제 수정
- [x] 인스타 관리 페이지 [돌아가기] 버튼 및 뒤로가기 작동하지 않는 문제 수정
- [x] 관리자 대시보드 모든 화면에 상단 네비게이션 바 표시

## 인스타 반응형 + 관리자 네비게이션 수정 (2026-02-15 추가)
...
## Notice 게시판 + 예약 게시판 개편 (2026-02-15 추가)
- [x] Notice View 페이지 하단 [다른 공지사항 보기] 리스트를 갤러리→리스트 형식으로 변경
- [x] 예약 게시판 카테고리 구조화: 담당자 정보, 행사 정보, 작업 내용, 결제 정보, 프로그램 및 요청사항
- [x] 예약 게시판 서브항목 추가 (담당자 성함/연락처/이메일, 행사명/장소/날짜/시간 등)
- [x] 작업 내용: 분류(사진촬영/공연촬영/영상제작/뮤직비디오/기타), 촬영유형, 특수요청, 공개여부
- [x] 결제 정보: 결제방식, 계산서발행, 견적액/결제금액/미납금액(관리자만 수정)
- [x] 프로그램 및 요청사항: content 박스
- [x] 파일 첨부: 기존 유지
- [x] 링크 첨부: 유효 링크 view에서 content에 표시
- [x] 카테고리 이름 관리자 대시보드에서 수정 가능
- [x] 서브항목 이름 관리자 대시보드에서 수정 가능 (별도 페이지)

## 예약 게시판 추가 개선 5건 (2026-02-15 추가)
- [x] View 페이지에서 이미지 첨부파일 인라인 표시
- [x] 수정 페이지에 특수 요청 체크박스 추가
- [x] 미납금액 자동 계산 (견적액 - 결제금액)
- [x] 결제 정보에 진행상황 항목 추가 (접수중/예약완료/준비중/작업중/작업완료/취소) + 관리자 설정 가능
- [x] 각 항목을 한 줄 레이아웃으로 변경 + 타이틀/내용 색상 구분

## 예약 게시판 버그 수정 3건 (2026-02-15 추가)
- [x] 진행상황이 리스트 페이지 및 메인 화면 latest에 반영되도록 수정
...
## 예약 게시판 UI 개선 (2026-02-15 추가 3)
- [x] 메인 화면 최근 예약 섹션에서 진행상황 표시를 날짜 우측으로 이동
- [x] 예약 게시판 리스트에 관리자 전용 체크박스 추가
- [x] 관리자 일괄 삭제 기능 구현 (서버 API + 프론트엔드)

## 기능 추가 (2026-02-15 추가 4)
- [x] 대시보드에 atelier01 게시판(admin_board) 링크 추가
...
## Atelier01 게시판 복원 마이그레이션 (2026-02-15 추가 6)
- [x] dameum.kr/bbs/board.php?bo_table=atelier01 게시판 데이터 스크래핑 (23개 게시물)
- [x] atelier01 카테고리로 posts 테이블에 데이터 마이그레이션 (23개 완료)
- [x] atelier01 게시판 전용 프론트엔드 페이지 구현 (/atelier01) - 목록/상세/페이지네이션
- [x] 네비게이션에 DIY Atelier 링크 연결 (데스크탑/모바일 모두)
- [x] 기존 관리자게시판(admin_board)은 그대로 유지 - 대시보드 탭명 '관리자게시판'으로 복원

## 예약 게시판 수정 버튼 (2026-02-15 추가 7)
- [x] 로그인한 글쓴이에게 예약 수정 버튼 표시 - userId 필드 추가, 글쓴이/관리자만 수정 가능, 삭제는 관리자만

<<<<<<< Updated upstream
## DIY Atelier 관리자 전용 및 이미지 수정 (2026-02-15 추가 8)
- [x] DIY Atelier 게시판을 관리자만 볼 수 있도록 변경 - 네비게이션 관리자 전용 + 페이지 자체 인증 체크
- [x] 게시물 본문의 {이미지:N} 태그를 실제 이미지로 교체 - 105개 이미지 S3 CDN 업로드, GNB5 손실 이미지 placeholder 처리
- [x] 예약 게시판 수정 버튼을 모든 사용자에게 항상 표시 - 삭제는 관리자만
- [x] 예약 상세 페이지에서 이메일 필드 완전 제거 (보기/편집 모두)
...
## Making Film 상세 및 예약 게시판 수정 (2026-02-15 추가 11)
- [x] Making Film 상세 페이지 하단 다른 게시물 보기를 3줄(3개)만 표시되도록 수정
- [x] 예약 게시판의 페이지당 게시물 수 설정이 적용되도록 수정 (5개로 변경 후 정상 표시 확인, 20개로 복원)

## 비밀번호 검증 기능 구현 (2026-02-15 추가 12)
- [x] 비로그인 사용자 예약 수정 시 비밀번호 검증 다이얼로그 구현
- [x] 서버에 reservations.verifyPassword API 추가
- [x] reservations.update에 권한 체크 로직 추가 (관리자/글쓴이는 바로 수정, 비회원은 비밀번호 필요)
- [x] 비밀번호 검증 성공 시 수정 모드 진입, 저장 시 guestPassword 전달

## Atelier01 게시판 관리자 CRUD (2026-02-15 추가 13)
- [x] 글쓰기 기능 추가 (RichTextEditor 연동, 제목/내용 입력)
- [x] 수정 기능 추가 (상세 페이지에서 수정 버튼 → 편집 폼)
- [x] 삭제 기능 추가 (상세 페이지에서 삭제 버튼 → 확인 후 삭제)
- [x] 목록 페이지에 글쓰기 버튼 추가
...
## 게시판 전체 폭 설정 view 페이지 반영 (2026-02-15 추가 16)
- [x] 게시판 레이아웃 설정의 '전체 폭' (컨테이너 최대 너비)이 view(상세) 페이지에도 반영되도록 수정 (container-wide를 인라인 스타일 maxWidth: 1536px로 적용)

## 게시판 레이아웃 설정 슬라이더 UI (2026-02-16 추가 17)
- [x] 전체 폭 설정을 드롭다운에서 슬라이더 바로 변경 (600px~1920px + 전체폭 토글)
- [x] 게시물의 폭 설정을 드롭다운에서 슬라이더 바로 변경 (10%~100% + 자동 토글)
- [x] 슬라이더 값이 실제 리스트/view 페이지에 반영되도록 연동 (px/% 값 직접 적용 확인)

## 이미지/영상 삽입 가능한 게시판 구현 (2026-02-16 추가 18)
- [ ] RichTextEditor에 파일 선택 이미지 업로드 버튼 추가 (파일 선택 → S3 업로드 → 커서 위치에 삽입)
- [ ] RichTextEditor에 YouTube 영상 임베드 버튼 추가 (URL 입력 → iframe 삽입)
- [ ] 삽입된 이미지/영상 드래그로 위치 이동 가능
- [ ] 임시 게시판 페이지 구현 (목록/글쓰기/상세/수정/삭제)
- [ ] 관리자 페이지에서 접근 가능하도록 라우트 및 네비게이션 추가
=======

## 미디어 게시판 이슈 수정 (2026-03-02 추가)
- [x] 미디어 게시판 글 저장 기능 - DB 마이그레이션 동기화 완료
- [x] YouTube URL 삽입 기능 - 정상 작동 확인
- [x] 파일 업로드 기능 - RichTextEditor에서 이미지/동영상 업로드 가능
- [x] BoardLayoutSettings 전체 게시판 표시 - 8개 게시판 모두 선택 가능하도록 확장
- [x] Tiptap 중복 확장 경고 수정 - StarterKit 3.x에 Link/Underline 포함되어 있으므로 제거
>>>>>>> Stashed changes
